...
class UserService {
    async registration(email, password, name, surname, ava) {
        ...
        const user = await UserModel.create({
            ...
            surname,
            email, 
            ava: ava || `${API_URL}images/common/init-ava.png`, 
    ...
    async updateUser(updates) {
        ...
        if(updates.password) {
            const isPassEquals = await bcrypt.compare(updates.password, user.password)
            if(!isPassEquals) {
                throw ApiError.BadRequest('Неверный пароль')
            }
        }

        const hashPassword = await bcrypt.hash(updates.newPassword || user.password, 3)
        let newUpdates = {}
